...
case $view_mode in
  1)  # World View
    ...
    if [[ $execute_world == "yes" ]]; then
        shodan search --limit 20 --fields ip_str,port --separator : 'title:"Internet Camera, live view, PelcoNet, +tm01+, PTZ Internet Camera, WVC210, CAMERA Viewer, NetCamXL Live Image, Live Images, webcamxp, webcam 7, dome camera, yawcam," -port:443 200 ok' | tee ips.txt
    else
  ...
  2)  # Country View
    ...
    grep -i "^$country_name,," locations.csv | while IFS=',' read -r country state city abbreviation; do
        ...
        shodan search --limit 20 --fields ip_str,port --separator : 'title:"Internet Camera, live view, PelcoNet, +tm01+, PTZ Internet Camera, WVC210, CAMERA Viewer, NetCamXL Live Image, Live Images, webcamxp, webcam 7, dome camera, yawcam" -port:443 200 ok -portcountry:'$country_abbr'' | tee -a ips.txt
    done
    ;;
  3)  # State View
    read -p "Enter the country name: " country_name
    ...
    grep -i "^$country_name,$state_name,," locations.csv | while IFS=',' read -r country state city abbreviation; do
        ...
        shodan search --limit 20 --fields ip_str,port --separator : 'title:"Internet Camera, live view, PelcoNet, +tm01+, PTZ Internet Camera, WVC210, CAMERA Viewer, NetCamXL Live Image, Live Images, webcamxp, webcam 7, dome camera, yawcam," -port:443 200 ok country:'$country_abbr' state:'$state_abbr'' | tee -a ips.txt
    done
    ;;
  4)  # City View
    read -p "Enter the country name: " country_name
    ...
    grep -i "^$country_name,$state_name,$city_name" locations.csv | while IFS=',' read -r country state city abbreviation; do
        ...
        shodan search --limit 20 --fields ip_str,port --separator : 'title:"Internet Camera, live view, PelcoNet, +tm01+, PTZ Internet Camera, WVC210, CAMERA Viewer, NetCamXL Live Image, Live Images, webcamxp, webcam 7, dome camera, yawcam," -port:443 200 ok country:'$country_abbr' state:'$state_abbr' city:'$city_abbr'' | tee -a ips.txt
    done
    ;;
  *)
    echo "Invalid choice."
